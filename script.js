document.addEventListener("DOMContentLoaded", function () {
  ...
  header.innerHTML = `
    ...
    <div class="header-actions">
      ...
      <button id="adminBtn" title="Panel de administrador">Admin</button>
    </div>
  `;
  ...
  var ADMIN_PASS = "sosdigital2026";

  var adminBtn = document.getElementById("adminBtn");
  if (adminBtn) {
    adminBtn.addEventListener("click", function () {
      if (sessionStorage.getItem("sos_admin")) {
        showAdminPanel();
      } else {
        showLoginModal();
      }
  ...
  function showLoginModal() {
    ...
    modal.innerHTML = `
      <div class="lightbox-content admin-login">
    ...
    modal.querySelector(".lightbox-close").addEventListener("click", function () { modal.remove(); });
    modal.addEventListener("click", function (e) { if (e.target === modal) modal.remove(); });

    document.getElementById("adminLoginForm").addEventListener("submit", function (e) {
      e.preventDefault();
      var u = document.getElementById("adminUser").value.replace(/[^a-zA-Z0-9]/g, "").toLowerCase();
      var p = document.getElementById("adminPass").value.replace(/[^a-zA-Z0-9]/g, "").toLowerCase();
  ...
  function showAdminPanel() {
    var allCardNames = [
      "Limpieza", "Cambiar RAM", "Pasta termica", "Disco duro", "Procesador",
      "Instalar Windows", "Instalar Linux", "Instalar Office", "Limpieza de Software", "Instalar Antivirus",
      "Inteligencia Artificial", "Dispositivos", "Futuro",
      "Partes PC", "Python", "Redes", "Mantenimiento", "Cables"
    ];
    var cardOptions = "";
    allCardNames.forEach(function (name) {
      cardOptions += '<option value="' + name + '">' + name + '</option>';
    });

    var modal = document.createElement("div");
    modal.className = "lightbox-overlay active";
    modal.innerHTML = `
      <div class="lightbox-content admin-panel">
        <button class="lightbox-close">&times;</button>
        <h2>Panel de Administrador</h2>
        <p>Sube documentos para que los usuarios puedan descargarlos.</p>
        <form id="uploadForm">
          <label>Tarjeta destino:</label>
          <select id="uploadCard" required>
            <option value="">Selecciona una tarjeta</option>
            ${cardOptions}
          </select>
          <label>Nombre del documento:</label>
          <input type="text" id="uploadTitle" placeholder="Ej: Guia de Limpieza" required>
          <label>Archivo (PDF, imagen, Word, etc):</label>
          <div class="file-input-wrap">
            <input type="file" id="uploadFile" accept=".pdf,.doc,.docx,.png,.jpg,.jpeg,.webp,.txt,.pptx,.xlsx" required>
            <span class="file-input-label">Buscar archivo...</span>
          </div>
          <p class="file-name-display" id="fileName">Ningun archivo seleccionado</p>
    ...
    document.getElementById("logoutBtn").addEventListener("click", function () {
      sessionStorage.removeItem("sos_admin");
      modal.remove();
    });

    document.getElementById("uploadForm").addEventListener("submit", function (e) {
      e.preventDefault();
      var cardName = document.getElementById("uploadCard").value;
      ...
      if (file.size > 4 * 1024 * 1024) { alert("Maximo 4 MB."); return; }
      var reader = new FileReader();
      reader.onload = function (ev) {
        var docs = JSON.parse(localStorage.getItem("sos_card_docs") || "[]");
        docs.push({ card: cardName, title: title, fileName: file.name, fileType: file.type, fileData: ev.target.result, date: new Date().toLocaleDateString() });
        localStorage.setItem("sos_card_docs", JSON.stringify(docs));
        renderDocList();
        e.target.reset();
        document.getElementById("fileName").textContent = "Ningun archivo seleccionado";
        alert("Documento subido a: " + cardName);
      };
      reader.readAsDataURL(file);
    });

    document.getElementById("uploadFile").addEventListener("change", function () {
      document.getElementById("fileName").textContent = this.files[0] ? this.files[0].name : "Ningun archivo seleccionado";
    });

    function renderDocList() {
      var list = document.getElementById("uploadedList");
      var docs = JSON.parse(localStorage.getItem("sos_card_docs") || "[]");
      if (docs.length === 0) { list.innerHTML = "<p>No hay documentos aun.</p>"; return; }
      list.innerHTML = docs.map(function (item, i) {
        return '<div class="upload-item"><span class="upload-badge">DOC</span><span>' + (item.fileName || item.title) + ' → ' + item.card + '</span><button class="upload-delete" data-index="' + i + '">&times;</button></div>';
      }).join("");
      list.querySelectorAll(".upload-delete").forEach(function (delBtn) {
        delBtn.addEventListener("click", function () {
          var docs = JSON.parse(localStorage.getItem("sos_card_docs") || "[]");
          docs.splice(parseInt(delBtn.dataset.index), 1);
          localStorage.setItem("sos_card_docs", JSON.stringify(docs));
          renderDocList();
        });
      });
